...
def Analysis(url):
    ...
    #print (length)
    return(sum1, sum1/packets,packets,mid_size/mid_packets)


url_list = ['https://en.wikipedia.org/wiki/Static_web_page']
...
for i in url_list:
    os.system('wget' + ' ' +i)
    time.sleep(5)
    total_len, length_per_pac, pac_len, length_per_mid_pac=Analysis(i)
with open('data_final2.json') as data_file:
    ...
    req_len={}
    req_len=defaultdict(lambda: [0,0], req_len)
    for d in data:
        urls = list(d.keys())
        url = urls[0]
        ...
        if(req_len[url][0]==0):
            req_len[url][0]=size
        elif(size<req_len[url][0]):
            req_len[url][0]=size
        elif(size>req_len[url][1]):
            req_len[url][1]=size
        size=size/num_pac
    ...
    for i in avg_pckt_len:
        print (i, avg_pckt_len[i], pac_count[i], mid_pckt_len[i], req_len[i])
    '''for i in avg_pckt_len:
        if((length_per_pac>=avg_pckt_len[i][0]) & (length_per_pac<=avg_pckt_len[i][1])):
            if((pac_len>=pac_count[i][0]) & (pac_len<=pac_count[i][1])):
                if((length_per_mid_pac>=mid_pckt_len[i][0]) & (length_per_mid_pac<=mid_pckt_len[i][1])):
                    final_list.append(i)'''
    for i in avg_pckt_len:
        if((total_len>=(req_len[i][0]-100)) & (total_len<=(req_len[i][1]+100))):
            final_list.append(i)
           
    #print (len(final_list))
    print (final_list.__len__())
    print (final_list)
    print (length_per_pac, pac_len, length_per_mid_pac, total_len)
    print (url_list[0])
